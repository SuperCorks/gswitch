...
Google blocks the default ADC OAuth client when you request Workspace scopes such as Drive, Gmail, Docs, Sheets, or Calendar. `gswitch` includes a production Desktop OAuth client ID for those ADC flows and uses it automatically with `gcloud auth application-default login`. The bundled file does not contain a client secret.

If you need to use a different OAuth client, pass a Desktop client JSON with `--client-id-file`. When that file contains a `client_secret`, `gswitch` also passes its client ID and secret to `gws auth login`. Recent `gws` versions can also use the ADC file generated by `gcloud auth application-default login`, so a separate `gws auth login` is not required for many Workspace CLI workflows.
